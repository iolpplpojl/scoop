    ...
    export function Connector({children}){
        ...
        const ConnectWs = () => {
            ...
            {
                socRef.current = new WebSocket("wss://121.130.189.100:9999/gateway");
        ...
        const verifyLogin = () =>
        {
            ...
            if(temptok != undefined){
                    ...
                    localStorage.removeItem("logintoken");
                        const doLogin = () => {
                            //axios로 로그인 요청, const trylogin = true일때 실행 X, response not ok면 다시 trylogin = false;, ok면 JWT 생성하고 메인으로 이동
                                axios("https://121.130.189.100:9999/api/RefreshAccess", {
                                    method : "get",
                                    params : {},
                ...
                if(temptok != null){
                        ...
                        const doVerify = () => {
                            //axios로 로그인 요청, const trylogin = true일때 실행 X, response not ok면 다시 trylogin = false;, ok면 JWT 생성하고 메인으로 이동
                                axios("https://121.130.189.100:9999/api/VerifyAccess", {
                                    method : "get",
                                    params : {
                                        key: temptok,
                                    },
